...
exports.handler = async (event) => {
  ...
  if(!code||!barcode) return {statusCode:400,headers:h,body:JSON.stringify({error:'Params requeridos'})};
  const f = FARMACIAS[code];
  if(!f||!f.activa) return {statusCode:403,headers:h,body:JSON.stringify({error:'No autorizado'})};
  return new Promise(resolve=>{
    const req=http.request({hostname:f.host,port:f.port,path:`/api/productos?codigoBarras=${encodeURIComponent(barcode)}`,method:'GET'},res=>{
      let d=''; res.on('data',c=>d+=c);
      res.on('end',()=>resolve({statusCode:res.statusCode,headers:h,body:d}));
    });
    req.on('error',e=>resolve({statusCode:502,headers:h,body:JSON.stringify({error:e.message})}));
    req.setTimeout(6000,()=>{req.destroy();resolve({statusCode:504,headers:h,body:JSON.stringify({error:'Timeout'})});});
    req.end();
  });
};
